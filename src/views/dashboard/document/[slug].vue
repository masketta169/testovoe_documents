<template>
    <div class="document">
        <img :src="imageSrc" class="document__img" @error="handleErrorImage" @load="imageError = false" />
        <div class="document__info">
            <WrapperActionItem :title="document?.name">
                <div class="document__actions">
                    <Button class="document__trigger" type="download" text="Скачать" @click="handleDownload" />
                    <Button class="document__trigger" @click="handleDelete" type="delete" text="Удалить" :disabled="isDisabledDelete" />
                </div>
            </WrapperActionItem>
...
const isDisabledDelete = computed(() => !documentsStore.getSelectedDocument?.image || imageError.value)

const imageSrc = ref(document.image)
