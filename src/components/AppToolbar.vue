<template>
    <div class="toolbar">
        <WrapperActionItem title="Поиск документа">
            <Input v-model="search" placeholder="Введите ID документа" @update:modelValue="debouncedFetchDocuments"/>
        </WrapperActionItem>
        <WrapperActionItem title="Поиск документа">
            <Loader :visible="isLoadingDocuments" />
            <div class="toolbar__documents">
                <CardDocument v-for="item in documentsStore.getDocuments" :key="item.id" v-bind="item" @click="openDocumentPath(item.id)" />
            </div>
        </WrapperActionItem>
    </div>
</template>
